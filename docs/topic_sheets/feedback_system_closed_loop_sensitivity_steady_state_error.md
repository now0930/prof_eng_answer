...
최종값이 존재하고 관련 극 조건이 만족될 때만 최종값 정리를 적용한다.

### 5.6 시스템 형

시스템 형은 단위 피드백 개루프 L(s)의 원점 극, 즉 순수 적분기의 수로 정의한다.

폐루프 차수나 폐루프 극 전체의 수로 정의하지 않는다.
...
| 시스템 형 | 계단 입력 | 램프 입력 | 포물선 입력 |
|---|---:|---:|---:|
| Type 0 | 1/(1+Kp_e) | ∞ | ∞ |
| Type 1 | 0 | 1/Kv | ∞ |
| Type 2 | 0 | 0 | 1/Ka |

폐루프 안정과 관련 오차상수의 존재를 전제로 한다.

### 5.9 적분과 trade-off

적분 동작은 정상상태 오차 제거에 기여할 수 있지만 위상여유 감소, 포화, windup 및 과도응답 악화를 유발할 수 있다.

루프 이득과 대역폭 증가는 추종성과 외란 억제를 개선할 수 있지만 지연, 미모델 동특성, 잡음 및 포화로 안정여유를 저하시킬 수 있다.
...
- [ ] 최종값 정리의 안정 조건이 포함되어 있는가?
- [ ] 시스템 형이 개루프 원점 극 수로 정의되어 있는가?
- [ ] Type별 오차표가 정확한가?
