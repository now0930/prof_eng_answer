...
정본은 `calibration/deterministic_authority_policy.json`이다. Stage58 Gemini-off 48건은 질문-only Topic routing recall 100%, known-fatal recall 100%(16/16), false positive 0, score coverage 100%(48/48), 점수 허용구간 적중률 85.42%, 평균 범위 이탈 0.35875점, known-overgrading 0건이다. 따라서 offline Authority Gate는 `READY`, 동일 입력 2회 Gate는 `STABLE`이다. 근거 artifact는 `reports/deterministic_replay_stage58_20260917.json`, `reports/deterministic_authority_stage58_20260917.json`, `reports/deterministic_stability_stage58_20260917.json`이다. 실제 Telegram 저장 세션의 provider-zero dry-run도 canonical 고유답안 60/60을 처리했다. 일반 합격은 질문 계약의 `pass_required_anchor_ids` 충족, 최소 만족 비율과 WRONG 없음이 필요하고, 고득점은 core gap과 심화 evidence Gate를 추가로 통과해야 한다. Stage39~41에서 provider-zero 운영 검증을 완료했으며 저장소 기본값은 rollback을 위해 `false`, 실제 운영 `.env`는 검증된 `true`를 사용한다.
