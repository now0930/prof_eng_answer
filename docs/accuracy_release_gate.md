...
정본은 `calibration/deterministic_authority_policy.json`이다. Stage56 Gemini-off 48건은 질문-only Topic routing recall 100%, known-fatal recall 100%(16/16), false positive 0, score coverage 100%(48/48), 점수 허용구간 적중률 85.42%, 평균 범위 이탈 0.392708점, known-overgrading 0건이다. 따라서 offline Authority Gate는 `READY`다. 근거 artifact는 `reports/deterministic_*_scope_calibration_20260912.json`이다. Stage55보다 적중률이 낮아진 주원인은 fatal 답안을 13점으로 끌어올리던 legacy floor를 제거해 실제 획득점만 남겼기 때문이다. 이는 fatal no-pass·ceiling 불변조건을 보존하면서 점수 부풀림을 제거한 의도된 migration이며, 과거 점수 구간 자체를 correctness로 간주하지 않는다. `scripts/audit_golden_risk_coverage.py` 기준 inventory는 78개 Topic, reviewed 48건, covered 31개, 3-lane 완료 6개다. 일반 합격은 질문 계약의 `pass_required_anchor_ids` 충족, 최소 만족 비율과 WRONG 없음이 필요하고, 고득점은 core gap과 심화 evidence Gate를 추가로 통과해야 한다. Stage39~41에서 provider-zero 운영 검증을 완료했으며 저장소 기본값은 rollback을 위해 `false`, 실제 운영 `.env`는 검증된 `true`를 사용한다.
