...
정본은 `calibration/deterministic_authority_policy.json`이다. Stage49 Gemini-off 33건은 질문-only Topic routing recall 100%, known-fatal recall 100%(10/10), false positive 0, score coverage 100%(33/33), 점수 허용구간 적중률 100%, 평균 범위 이탈 0점, known-overgrading 0건이다. 따라서 offline Authority Gate는 `READY`다. `scripts/audit_golden_risk_coverage.py`가 78개 Topic의 normal/adverse/fatal lane 공백을 위험도 순으로 제시하며, 첫 FSRM Topic은 세 lane을 모두 확보했다. 심화 증거 미충족 답안은 18.5점, 일반 자동 증거만으로 산정 가능한 최고점은 24점으로 제한한다. Stage39~41에서 provider-zero 운영 검증을 완료했으며 저장소 기본값은 rollback을 위해 `false`, 실제 운영 `.env`는 검증된 `true`를 사용한다.
