...
정본은 `calibration/deterministic_authority_policy.json`이다. Stage48 Gemini-off 30건은 질문-only Topic routing recall 100%, known-fatal recall 100%(9/9), false positive 0, score coverage 100%(30/30), 점수 허용구간 적중률 100%, 평균 범위 이탈 0점, known-overgrading 0건이다. 따라서 offline Authority Gate는 `READY`다. 심화 증거 미충족 답안은 18.5점, 일반 자동 증거만으로 산정 가능한 최고점은 24점으로 제한한다. 25점은 별도의 결정론적 exceptional-evidence contract가 구현되기 전까지 자동 부여하지 않는다. Stage39~41에서 current-code Gate, container parity/replay, legacy reconciler 차단 및 실제 Telegram raw/final exact match를 완료했다. 저장소 기본값은 rollback을 위해 `false`, 실제 운영 `.env`는 검증된 `true`를 사용한다.
