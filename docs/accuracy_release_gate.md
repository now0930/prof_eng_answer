...
정본은 `calibration/deterministic_authority_policy.json`이다. Stage55 Gemini-off 48건은 질문-only Topic routing recall 100%, known-fatal recall 100%(16/16), false positive 0, score coverage 100%(48/48), 점수 허용구간 적중률 93.75%, 평균 범위 이탈 0.14125점, known-overgrading 0건이다. 따라서 offline Authority Gate는 `READY`다. 근거 artifact는 `reports/deterministic_*_scope_calibration_20260910.json`이다. `scripts/audit_golden_risk_coverage.py` 기준 inventory는 78개 Topic, reviewed 48건, covered 31개, 3-lane 완료 6개다. 고득점은 core gap 없음, WRONG 없음, 만족 비율과 누락 비율 Gate를 추가로 통과해야 한다. Stage39~41에서 provider-zero 운영 검증을 완료했으며 저장소 기본값은 rollback을 위해 `false`, 실제 운영 `.env`는 검증된 `true`를 사용한다.
