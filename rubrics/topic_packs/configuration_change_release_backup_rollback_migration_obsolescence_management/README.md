# 형상관리, 변경관리, Release, Backup, Rollback, Migration 및 Obsolescence 관리
...
- Deterministic Logic Check: disabled
- Generated rebuild, release promotion, commit and push: not performed
...
SW-04는 요구사항, 설계, 구현, 시험, 독립 V&V와 개발 수명주기 자체를 담당한다.

SW-06은 운영 중 승인된 형상에 대한 변경요청, 영향분석, 승인, Release, Regression, Backup, Restore, Rollback, Migration과 수명주기 관리를 담당한다.

SW-04의 시험결과는 SW-06 Release acceptance의 입력이 될 수 있으나 ownership은 분리한다.

### SW-09와의 경계

SW-06의 Backup과 Recovery는 변경 실패, 장비고장, 운전복구와 As-built consistency를 중심으로 한다.

SW-09는 공격, 악성코드, 권한침해, 방어통제와 사이버 사고대응을 중심으로 한다.
...
- `PLC 프로그램 변경 승인`
- `DCS database release`
- `HMI project version`
- `SCADA configuration backup`
- `engineering workstation restore`
- `firmware upgrade compatibility`
- `license dongle recovery`
- `historian database migration`
- `alarm database migration`
- `recipe and batch data migration`
- `OPC interface cutover`
- `field device configuration backup`
- `checksum and release manifest`
- `maintenance window go no-go`
- `online change and emergency change`
- `as-built drawing update`
- `site acceptance after release`
- `rollback trigger and time limit`
- `restore drill and recovery evidence`
- `RTO RPO and fallback operation`
- `parallel run control ownership`
- `legacy hardware spare strategy`
- `end of support and last-time buy`
- `vendor tool dependency`
- `data export and portability`
- `driver OS application compatibility`
...
- Source JSON 4개 schema와 topic_id 일치
- Fact Anchor 40개와 unique ID
- Fatal 20개, Major 12개
...
- Baseline·latest file distinction
- Firmware compatibility and Obsolescence lifecycle
- Generated와 공통 Router 미수정
...
이 Topic Pack은 source authoring만 수행한다. `rubrics/generated/**`, 공통 Router, production Python, release 공통 script와 다른 Topic은 수정하지 않는다.
