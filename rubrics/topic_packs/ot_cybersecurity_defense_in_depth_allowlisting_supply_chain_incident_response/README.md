...
- **sw09_secure_remote_session** — 원격 세션은 암호화, 명시적 승인, 화면·명령 기록, file transfer 통제, concurrent session 제한, inactivity timeout과 비상차단을 적용해야 한다.
...
- **sw09_exercise_metrics** — Tabletop, technical drill과 recovery exercise는 realistic scenario로 역할·의사결정·통신·복구를 시험하고 MTTD, MTTC, recovery success와 overdue action을 개선지표로 관리해야 한다.
