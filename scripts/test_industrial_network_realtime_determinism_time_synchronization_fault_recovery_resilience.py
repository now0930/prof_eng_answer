...
class TestSW08Metadata(unittest.TestCase):
    ...
    def test_question_type_consistency(self) -> None:
        self.assertEqual(FACT["question_type_hint"], "CALC_DESIGN")
        self.assertEqual(MODEL["question_type"], "CALC_DESIGN")
        self.assertEqual(IMPORTANCE["question_type"], "CALC_DESIGN")
        self.assertEqual(LOGIC["deterministic_checks"]["question_type"], "CALC_DESIGN")
...
class TestSW08FactContracts(unittest.TestCase):
    ...
    def test_realtime_and_redundancy_markers(self) -> None:
        combined = json.dumps(FACT, ensure_ascii=False)
        for marker in ("Latency","Jitter","Cycle time","Determinism","TSN","PTP","MRP","DLR","PRP","HSR","Graceful degradation"):
            self.assertIn(marker, combined)

    def test_determinism_is_bounded_not_average(self) -> None:
        anchor = next(row for row in FACT["anchors"] if row["id"] == "sw08_determinism_definition")
        self.assertIn("상한", anchor["statement"])
        self.assertIn("평균속도", anchor["statement"])

    def test_source_basis_has_primary_reference_families(self) -> None:
        combined = " ".join(row["source_basis"] for row in FACT["anchors"])
        self.assertIn("IEC 61784-2", combined)
        self.assertIn("IEEE 1588", combined)
        self.assertIn("IEC 62439-3", combined)
        self.assertIn("IEEE 802.1Qbv", combined)

class TestSW08LogicContracts(unittest.TestCase):
    ...
    def test_false_positive_cautions_cover_conditional_cases(self) -> None:
        ...
        for marker in ("TSN","PTP","MRP","PRP","Graceful degradation"):
            self.assertIn(marker, cautions)

class TestSW08ModelContracts(unittest.TestCase):
    ...
    def test_routing_aliases_are_narrow(self) -> None:
        ...
        forbidden_exact = {"network","Ethernet","realtime","latency","jitter","security","communication","redundancy"}
        self.assertTrue(forbidden_exact.isdisjoint(set(aliases)))

    def test_routing_fields_cover_realtime_resilience(self) -> None:
        fields = " ".join(MODEL["routing_field_points"])
        for marker in ("TSN","PTP","MRP","DLR","PRP","HSR","fault injection"):
...
class TestSW08BoundaryAndDocuments(unittest.TestCase):
    def test_sw07_boundary_explicit(self) -> None:
        for text in (README, SHEET_TEXT):
            self.assertIn("SW-07", text)
            self.assertIn("Protocol", text)
            self.assertIn("상호운용성", text)
...
if __name__ == "__main__":
    ...
    print(f"SW08_FOCUSED_TEST_COUNT={count}")
