...
PATTERNS = {
    "direct_fact_anchor_runtime_path": r"rubrics[/\"]\s*/?\s*[\"']?fact_anchors|rubrics/fact_anchors",
    "industrial_fact_anchor_file": r"fact_anchors.+industrial_instrumentation_control\.json|industrial_instrumentation_control\.json.+fact_anchors",
    "fact_anchor_constant": r"FACT_ANCHOR|ANCHOR_BANK|FACT_BANK",
    "fact_anchor_loader": r"load_fact_anchor_bank|save_fact_anchor_bank",
    "fact_anchor_text": r"fact_anchor|fact anchors|Fact Anchor|FACT ANCHOR",
    "resolver_fact_anchors": r"resolve_rubric_bank_path\([\"']fact_anchors[\"']\)",
}
...
def classify(file: str, pattern: str, text: str) -> str:
    ...
    if pattern in {
        "direct_fact_anchor_runtime_path",
        "industrial_fact_anchor_file",
        "fact_anchor_constant",
    }:
        return "direct_runtime_candidate"

    if pattern == "resolver_fact_anchors":
        return "already_uses_resolver"

    if pattern == "fact_anchor_loader":
...
def write_markdown(path: Path, report: dict[str, Any]) -> None:
    ...
    lines.append("# Fact Anchor Usage Audit")
    lines.append("")
    lines.append(f"- generated_at: `{report['generated_at']}`")
    lines.append("- purpose: fact anchor runtime path를 직접 읽는 코드와 resolver 적용 후보를 찾는다.")
    lines.append("")
...
def main() -> int:
    ...
    json_path = REPORT_DIR / f"fact_anchor_usage_audit_{timestamp}.json"
    md_path = REPORT_DIR / f"fact_anchor_usage_audit_{timestamp}.md"

    write_json(json_path, report)
    write_markdown(md_path, report)

    print()
    print("FACT ANCHOR USAGE AUDIT OK")
