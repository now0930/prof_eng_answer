...
OUT_MD = ROOT / "reports/model_answer_remaining_p2_order_context.md"
OUT_CSV = ROOT / "reports/model_answer_remaining_p2_order_context.csv"
...
def main() -> int:
    ...
    targets = [
        ...
        and r.get("action") in {"order_fix", "weak_outline_item_review"}
    ]
    ...
    out_rows = []
    md = []
    md.append("# Remaining P2 Order / Weak Outline Context")
    md.append("")
    ...
    for aid in answer_ids:
        ...
        if not a:
            md.append(f"## {aid}")
            md.append("")
            md.append("- ERROR: answer not found")
            md.append("")
            continue
